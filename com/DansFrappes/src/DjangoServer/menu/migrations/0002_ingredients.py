...
def add_inventory(apps, schema_editor):
    ...
    matcha.save()
    whipped_cream = Ingredient(name="Whipped Cream", stock=1000, buy_cost=0.20, sell_cost=0.35)
    whipped_cream.save()

    ## Milks (10 stock is 1 gallons)
    milk_whole = MilkIngredient(name="Whole", stock=100, buy_cost=0.20, sell_cost=3.50)
    milk_whole.save()
    milk_skim = MilkIngredient(name="Skim", stock=100, buy_cost=0.20, sell_cost=3.50)
    milk_skim.save()
    milk_almond = MilkIngredient(name="Almond", stock=100, buy_cost=0.25, sell_cost=3.80)
    milk_almond.save()
    milk_cashew = MilkIngredient(name="Cashew", stock=100, buy_cost=0.35, sell_cost=4.00)
    milk_cashew.save()
    milk_oat = MilkIngredient(name="Oat", stock=100, buy_cost=0.37, sell_cost=4.10)
    milk_oat.save()
